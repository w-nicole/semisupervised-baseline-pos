...
class LSTMLinear(torch.nn.Module):
    ...
    def pack_padded_input(self, lengths, padded_input):
        return util.apply_gpu(torch.nn.utils.rnn.pack_padded_sequence(padded_input, lengths.cpu(), batch_first=True, enforce_sorted=False))

    def forward(self, lengths, lstm_input_raw):
        lstm_input = self.pack_padded_input(lengths, lstm_input_raw)
