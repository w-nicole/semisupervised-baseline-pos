    ...
    try:
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                args = (batch["sent"], batch["start_indices"], batch["end_indices"], batch["lang"])
                args = tuple(list(map(util.apply_gpu, args)))
                hs = model.encode_sent(*args)
                # end taken
                flatten_data = lambda tensor : tensor.reshape(-1, clean_states.shape[-1])
                flat_states = flatten_data(clean_states)
                all_states.append(flat_states)
                all_padded_labels.append(batch['labels'])
