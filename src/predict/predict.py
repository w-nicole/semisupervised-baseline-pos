...
def prediction_to_accuracy(model, predictions, checkpoint_path):
    ...
    for lang, raw_logits in predictions.items():
        ...
        outputs, labels = predict_utils.clean_padded_labels_and_predictions(model, lang, flat_predicted_labels)
        accuracy = (torch.sum(outputs == labels) / outputs.shape[0]).item()
    
    return accuracy
    
def get_predictions(model, language, analysis_path):
    ...
    if not os.path.exists(predictions_path):
        raw_predictions = get_all_predictions(model, [language])
