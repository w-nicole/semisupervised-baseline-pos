...
def predict_over_languages(checkpoint_path, model_class):
    ...
    analysis_parent_path = os.path.join(predict_utils.get_analysis_path(checkpoint_path), 'val_predictions')
    accuracies = {}
    for language in languages:
        print(language)
        analysis_path = os.path.join(analysis_parent_path, language)
        raw_predictions = predict.get_predictions(model, language, analysis_path)
        accuracy = predict.prediction_to_accuracy(model, raw_predictions, checkpoint_path)
        accuracies[language] = accuracy
    
    df = pd.DataFrame.from_records([accuracies])
    df.to_csv(os.path.join(analysis_parent_path, 'val_accuracies.csv'))
    return df
...
if __name__ == '__main__':
    ...
    for model_type in model_names:
        ...
        model_path = os.path.join('./experiments/sweeps/seed', model_type)
        for seed in seeds:
            ...
            df = predict_over_languages(checkpoint_path, Tagger if model_type == 'baseline' else LatentBase)
