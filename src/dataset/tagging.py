...
class TaggingDataset(Dataset):
    ...
    def yield_postprocessed_outputs(self, raw_token_ids, raw_label_ids):
        ...
        assert token_ids.shape == label_ids.shape, f'tokens: {token_ids.shape}, labels: {label_ids.shape}'
        # Note: below non_pad_length includes the [SEP].
        return [(token_ids, label_ids, label_ids.shape[0])]
    ...
    def process_example(self, example: Dict) -> List[Dict]:
        ...
        if not sent:
            ...
            output_keys = ['sent', 'pos_labels', 'non_pad_length']
